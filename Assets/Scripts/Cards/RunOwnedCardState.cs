...
{
    [Serializable]
    public sealed class RunOwnedCardState
    {
        ...
        public bool TryAdd(RunCardInstance card, out RunDeckFailure failure)
        {
        ...
        {
            if (string.IsNullOrWhiteSpace(ownedCardId)) return null;
            return cards.Find(card => card != null && string.Equals(
                card.OwnedCardId, ownedCardId,
