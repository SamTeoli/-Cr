    ...
    internal static class BattleRuntimeFullRegressionValidation
    {
        ...
        private static void ValidateFromMenu()
        {
            ...
            EditorUtility.DisplayDialog(
                ...
                valid
                    ? "Full battle runtime regression C01-C12, unified resume routing, and settlement passed."
        ...
        internal static bool Validate()
        {
            ...
            valid &= Run(
                "Complete battle runtime rounds",
                BattleRuntimeRoundServiceValidation.Validate);
            valid &= Run(
                "Multi-round battle runtime sessions",
                BattleRuntimeSessionServiceValidation.Validate);
            valid &= Run(
            ...
            valid &= Run(
                "Victory consumable reward and elite progression",
                BattleVictoryConsumableRewardServiceValidation.Validate);
            valid &= Run(
                "Victory permanent reward and final boss progression",
                BattleVictoryPermanentRewardServiceValidation.Validate);
            valid &= Run(
                "Permanent reward save and load",
                PlayerPermanentRewardSaveServiceValidation.Validate);
            valid &= Run(
                "Run progress save and load",
                RunProgressSaveServiceValidation.Validate);
            valid &= Run(
            ...
            valid &= Run(
                "Checkpoint encounter resolution",
                ActiveBattleCheckpointEncounterResolutionValidation.Validate);
            valid &= Run(
                "Unified run resume routing",
                RunResumeServiceValidation.Validate);
